...
// the pitch of X-axle is 1 mm and one pulse move the axle with 1.8 degree.
...
static uint16_t used_pixel[3648];  // the largest length of used pixel is 3648
static uint16_t pos_space[3648];   // the storage space of position


/**
 * Function: filter the black pixel in raw data
 * Input: seam_location, the point of the raw data
 *        light_data, the point of the valid data, output point
...
* beta as in equ.15.8.8, and calculate \chi^2.
...
* alamda<0 for initialization (which then sets alamda=.001).
...
* repeatedly until convergence is achieved.
*
...
*  Find the maximum position by bubble sort method.
* Input: measure data
*     x: position
*     y: light intensity
